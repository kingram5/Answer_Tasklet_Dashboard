    ...
    <div>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        <input
          ...
          className="flex-1 min-w-[200px] bg-dark-700 border border-white/[0.06] rounded-md px-3 py-1.5 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-teal-500/50"
        />
      </div>
      <FilterBar>
        {['all', 'action', 'fyi', 'spam'].map(f => (
          <FilterPill key={f} active={filter === f} onClick={() => setFilter(f)}>
            {f === 'all' ? 'All' : CLASS_LABEL[f] || f}
          </FilterPill>
        ))}
      </FilterBar>
      ...
      {!loaded ? (
        ...
        <div className="flex gap-4">
          {/* Email list */}
          <div className="flex-1 space-y-2 min-w-0 card-stagger">
            {sorted.length === 0 && (
              ...
              return (
                <Card
                  ...
                  className={`p-4 cursor-pointer ${
                    isSelected ? 'border-teal-500/50 glow-border-teal' : ''
                ...
                >
                  ...
                  {/* AI Summary */}
                  {email.ai_summary && (
                    <p className="text-sm text-gray-300 leading-relaxed mb-2">{email.ai_summary}</p>
                  )}
                    ...
                    <div className="flex flex-wrap gap-1.5 mb-2">
                      {attachments.map((att, i) => (
                        <a
                          ...
                          className="inline-flex items-center gap-1 px-2 py-0.5 bg-dark-700 border border-white/[0.06] rounded text-xs text-gray-400 hover:text-teal-400 hover:border-teal-500/30 transition-colors"
                          title={att.filename}
                        >
                          <span>📎</span>
                  ...
                  <div className="flex items-center gap-2">
                    <select
                      ...
                      onClick={(e) => e.stopPropagation()}
                      className="bg-dark-700 border border-white/[0.06] rounded px-1.5 py-0.5 text-xs text-gray-400 focus:outline-none"
                    >
          ...
          {selected && (
            <div className="hidden md:block w-96 shrink-0 glass-dense rounded-lg p-4 sticky top-4 self-start max-h-[80vh] overflow-y-auto scrollbar-thin">
              <button
              ...
              {selected.ai_summary && (
                <div className="bg-dark-700 border border-white/[0.06] rounded-md p-3 mb-3">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {getAttachments(selected).map((att, i) => (
                      <a
                        ...
                        className="inline-flex items-center gap-1 px-2 py-1 bg-dark-700 border border-white/[0.06] rounded text-xs text-gray-400 hover:text-teal-400 hover:border-teal-500/30 transition-colors"
                      >
                        <span>📎</span>
